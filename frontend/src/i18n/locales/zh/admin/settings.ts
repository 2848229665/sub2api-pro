export default {
    settings: {
      ...
      openaiPrioritySaturation: {
        title: 'OpenAI 自适应双池调度',
        description: '低并发时以账号池为主，高并发时账号池与 API Key 池按实时容量共同调度。',
        enable: '启用自适应双池调度',
        enableHint: '该策略优先级最高；可与旧调度开关同时开启，关闭后再回落到实验加权或旧调度。',
        poolBalanceEnabled: '启用账号 / Key 池平衡',
        poolBalanceEnabledHint: '默认开启。低并发按配置比例分配；账号池进入高负载后按两池实时剩余容量动态分配。',
        accountSharePercentLabel: '低并发账号池占比',
        accountSharePercentHint: '默认 2:1 中的账号侧，即 67%；与 Key 池占比合计必须为 100%。',
        apiKeySharePercentLabel: '低并发 Key 池占比',
        apiKeySharePercentHint: '默认 2:1 中的 Key 侧，即 33%；保持“账号、账号、Key”的低流量节奏。',
        poolSharePercentError: '账号池与 Key 池占比都必须是 1 到 99 的整数，且合计为 100%。',
        enterHighLoadPercentLabel: '进入高并发阈值',
        enterHighLoadPercentHint: '默认 70%。账号池使用率达到该值或已有排队时进入高并发模式。',
        exitHighLoadPercentLabel: '退出高并发阈值',
        exitHighLoadPercentHint: '默认 50%。低于该值且无排队持续 5 秒后退出高并发模式。',
        loadThresholdPercentError: '进入阈值必须为 1 到 100，退出阈值必须为 0 到 99，且退出阈值小于进入阈值。',
        reservePercentLabel: '亲和会话预留百分比',
        ...
        capacityWarning: 'R = floor(C × 百分比 / 100)，G = C - R。C=0 表示无限并发，此时 R=0；高并发模式带 5 秒迟滞，避免阈值附近频繁切换。'
      },
