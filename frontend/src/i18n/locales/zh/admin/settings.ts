export default {
    settings: {
      ...
      openaiPrioritySaturation: {
        ...
        description: '按 Priority 升序、账号 ID 升序确定性选取，前一账号的新会话普通容量满载后才使用下一账号。',
        enable: '启用优先级饱和调度',
        enableHint: '该策略优先级最高；可与旧调度开关同时开启，关闭后再回落到实验加权或旧调度。',
        poolBalanceEnabled: '启用账号 / Key 池平衡',
        poolBalanceEnabledHint: '新会话按账号约三分之二、Key 约三分之一分配；账号池饱和时动态增加 Key 并发。',
        apiKeySharePercentLabel: 'Key 初始并发比例',
        apiKeySharePercentHint: '默认 33%，仅影响新会话；高负载时会根据账号池溢出量动态调整。',
        apiKeySharePercentError: 'Key 初始并发比例必须是 1 到 99 之间的整数。',
        ...
        capacityWarning: 'R = floor(C × 百分比 / 100)，G = C - R。C=0 表示无限并发，此时 R=0；排在前面的无限账号不会因容量满载而自然扩散。'
      },
