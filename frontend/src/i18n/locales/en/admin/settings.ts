export default {
    settings: {
      ...
      openaiPrioritySaturation: {
        ...
        description: 'Accounts are selected deterministically by ascending Priority, then ascending account ID; a new session moves on only after the preceding account’s general capacity is full.',
        ...
        poolBalanceEnabledHint: 'New sessions target roughly two-thirds account traffic and one-third API key traffic; API key concurrency expands when the account pool saturates.',
        apiKeySharePercentLabel: 'Initial API key concurrency share',
        apiKeySharePercentHint: '33% by default. It applies to new sessions; high load can raise the effective share when account capacity overflows.',
        apiKeySharePercentError: 'The API key concurrency share must be an integer from 1 through 99.',
        ...
        capacityWarning: 'R = floor(C × percentage / 100), and G = C - R. C=0 means unlimited concurrency and therefore R=0; an earlier unlimited account will not naturally spill over due to capacity.'
      },
