export default {
    settings: {
      ...
      openaiPrioritySaturation: {
        title: 'OpenAI adaptive dual-pool scheduler',
        description: 'At low concurrency the account pool is primary; at high concurrency the account and API key pools share traffic according to live capacity.',
        enable: 'Enable adaptive dual-pool scheduling',
        enableHint: 'This policy has highest precedence. It may remain enabled with legacy scheduler switches; disabling it falls back to the weighted or legacy scheduler.',
        poolBalanceEnabled: 'Enable account / API key pool balancing',
        poolBalanceEnabledHint: 'Enabled by default. Low concurrency follows the configured shares; high concurrency uses each pool’s live remaining capacity.',
        accountSharePercentLabel: 'Low-load account pool share',
        accountSharePercentHint: 'The account side of the default 2:1 split: 67%. Account and API key shares must add up to 100%.',
        apiKeySharePercentLabel: 'Low-load API key pool share',
        apiKeySharePercentHint: 'The API key side of the default 2:1 split: 33%, preserving an account, account, API key rhythm at low volume.',
        poolSharePercentError: 'Account and API key shares must both be integers from 1 through 99 and must add up to 100%.',
        enterHighLoadPercentLabel: 'Enter high-concurrency threshold',
        enterHighLoadPercentHint: '70% by default. High-concurrency mode starts when account-pool utilization reaches this value or requests are queued.',
        exitHighLoadPercentLabel: 'Exit high-concurrency threshold',
        exitHighLoadPercentHint: '50% by default. High-concurrency mode exits after utilization stays below this value with no queue for 5 seconds.',
        loadThresholdPercentError: 'The enter threshold must be 1–100, the exit threshold must be 0–99, and exit must be lower than enter.',
        reservePercentLabel: 'Affinity session reserve percentage',
        ...
        capacityWarning: 'R = floor(C × percentage / 100), and G = C - R. C=0 means unlimited concurrency and therefore R=0. A 5-second hysteresis prevents rapid mode flapping near the thresholds.'
      },
