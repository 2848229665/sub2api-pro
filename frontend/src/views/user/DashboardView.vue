...
import { getUsageTrendGranularityForRange } from '@/utils/dateRanges'
...
const startDate = ref(formatDateLocalInput(new Date(Date.now() - 6 * 86400000))); const endDate = ref(formatDateLocalInput(new Date())); const granularity = ref<UsageTrendGranularity>('day')
...
const onDateRangeChange = (range: { startDate: string; endDate: string }) => { granularity.value = getUsageTrendGranularityForRange(range.startDate, range.endDate); void loadCharts() }
