...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if endpoint.IsGenerationRequest() && !groupModelsListAllowsModel(c, apiKey, requestModel) {
		h.errorResponse(c, http.StatusNotFound, "model_not_found", groupModelsListModelNotFoundMessage(c, requestModel))
		return
	}
	...
	for {
		...
		var resolvedSessionHash string
		accountReleaseFunc, slotResult, resolvedSessionHash = h.acquireResponsesAccountSlot(c, apiKey.GroupID, admissionSessionHash, selection, true, false, &streamStarted, reqLog)
