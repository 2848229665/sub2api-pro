...
func TestOpenAIGatewayServiceNativeCodexOAuthIdentityDoesNotDependOnOptimization(t *testing.T) {
	...
	wantScopedSession := scopeCodexAccountIdentityValue(account, 44, "session", "default-session")
	wantScopedThread := scopeCodexAccountIdentityValue(account, 44, "thread", "default-thread")
	require.Equal(t, wantScopedSession, gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
	require.Equal(t, wantScopedSession, gjson.GetBytes(upstream.lastBody, "client_metadata.session_id").String())
	require.Equal(t, wantScopedThread, gjson.GetBytes(upstream.lastBody, "client_metadata.thread_id").String())
