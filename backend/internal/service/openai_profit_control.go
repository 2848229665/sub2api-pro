...
// bindOpenAIStickySessionDuringSelection preserves the Fork's eager canonical
// claim behavior for requests without a profit gate. Profit-controlled callers
// that use this helper defer the claim until the terminal post-slot check.
...
// account. Without a profit gate it performs the Fork's idempotent canonical
// claim, so a handler-side duplicate bind cannot replace a concurrent owner.
// With a gate it also never overwrites a different binding that already exists,
// so a temporarily ineligible account remains sticky and becomes eligible again
// automatically after its rate recovers.
