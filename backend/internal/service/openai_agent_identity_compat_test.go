...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	// 按 API Key 的会话隔离已移除：session/conversation 头透传客户端原始值。
	...
	// Authentication mode must not affect session headers or prompt-cache
	// behavior. Compare the same request with the existing OAuth path.
