...
func TestOpenAIWSHTTPBridgeStripsResponsesLiteForUnsupportedModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			`data: {"type":"response.completed","response":{"id":"resp_bridge_full","model":"gpt-5.5","usage":{"input_tokens":1,"output_tokens":1}}}`,
			"",
		}, "\n"))),
	}}
	...
	account := &Account{
		ID: 8, Name: "oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	payload := []byte(`{"type":"response.create","generate":true,"model":"gpt-5.5","stream":true,"client_metadata":{"ws_request_header_x_openai_internal_codex_responses_lite":"true"},"input":"hi"}`)
