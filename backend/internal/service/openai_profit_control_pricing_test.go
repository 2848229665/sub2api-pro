...
// 无门时准入后绑定保持 Fork 的 canonical-claim 语义：空会话会立即认领，
// 已有 owner 不会被 handler 的重复绑定覆盖（评审 M-Bind 回归锚点）。
func TestOpenAIProfitControlAfterAdmissionBindUsesCanonicalClaimWithoutGate(t *testing.T) {
	groupID := int64(82)
	...
	require.Equal(t, expensiveID, cache.sessionBindings[cacheKey], "handler 重复绑定不得覆盖 Fork 的 canonical owner")

	delete(cache.sessionBindings, cacheKey)
	require.NoError(t, svc.BindStickySessionAfterProfitAdmission(context.Background(), &groupID, sessionHash, cheapID))
	require.Equal(t, cheapID, cache.sessionBindings[cacheKey], "无 owner 时应在准入后建立粘性绑定")
}
