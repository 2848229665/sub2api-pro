...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	explicitPromptCacheKeyProvided := promptCacheKey != ""
	...
	sessionPromptCacheKey = promptCacheKey
	if metadataSessionCacheKey := promptCacheKeyFromAnthropicMetadataSession(&anthropicReq); metadataSessionCacheKey != "" {
		sessionPromptCacheKey = metadataSessionCacheKey
	}
	setOpenAICodexRelaySessionSeed(c, sessionPromptCacheKey)
	...
	if account.UsesOpenAICodexProtocol() && account.Platform != PlatformGrok {
		...
		if explicitPromptCacheKeyProvided {
	...
	if account.Platform != PlatformGrok &&
		...
		bodyPromptCacheKey := strings.TrimSpace(promptCacheKey)
		if account.Type == AccountTypeOAuth && explicitPromptCacheKeyProvided {
			if identity, ok := openAICodexUpstreamIdentityFromContext(c); ok && identity.SessionID != "" {
				bodyPromptCacheKey = identity.SessionID
			}
		}
		if bodyPromptCacheKey != "" {
			...
			if existing, ok := reqBody["prompt_cache_key"].(string); !ok || strings.TrimSpace(existing) == "" {
				reqBody["prompt_cache_key"] = bodyPromptCacheKey
	...
	if account.UsesOpenAICodexProtocol() && account.Platform != PlatformGrok {
		identity, identityErr := resolveOpenAICodexUpstreamIdentityWithSessionID(c, account, sessionPromptCacheKey)
		if identityErr != nil {
			return nil, fmt.Errorf("prepare messages bridge Codex identity: %w", identityErr)
		}
		if identity != (openAICodexUpstreamIdentity{}) && c != nil {
	...
	if account.Platform == PlatformGrok {
		...
		upstreamReq, err = s.buildUpstreamRequest(upstreamCtx, c, account, responsesBody, token, isStream, sessionPromptCacheKey, false)
	}
	releaseUpstreamCtx()
	if err != nil {
	...
	if account.Platform != PlatformGrok && sessionPromptCacheKey != "" {
		if identity, ok := openAICodexUpstreamIdentityFromContext(c); ok && identity.SessionID != "" {
			setOpenAIUpstreamSessionHeaders(upstreamReq.Header, identity.SessionID)
			if upstreamReq.Header.Get("conversation_id") != "" {
				upstreamReq.Header.Set("conversation_id", identity.SessionID)
			}
		} else {
			isolatedSessionID := generateSessionUUID(isolateOpenAIUpstreamSessionID(apiKeyID, codexAccountIdentitySource(c, account), sessionPromptCacheKey))
