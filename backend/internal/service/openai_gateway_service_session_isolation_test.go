...
func TestIsolateOpenAISessionID(t *testing.T) {
	...
	t.Run("format_is_32_hex_chars", func(t *testing.T) {
		result := isolateOpenAISessionID(99, "test_session")
		assert.Len(t, result, 32, "sha256 前 16 字节应为 32 字符 hex")
	...
	t.Run("zero_apiKeyID_returns_raw", func(t *testing.T) {
		assert.Equal(t, "session", isolateOpenAISessionID(0, "session"))
		assert.Equal(t, "session", isolateOpenAISessionID(-1, "session"))
