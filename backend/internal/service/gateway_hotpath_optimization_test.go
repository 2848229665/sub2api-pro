...
func TestInvalidateAvailableModelsCache_ByDimensions(t *testing.T) {
	...
	svc.modelsListCache.Set(modelsListCacheKey(&group9, accessibleModelsCacheNamespace+PlatformAnthropic), []string{"accessible-a"}, time.Minute)
	...
	t.Run("invalidate_group_and_platform", func(t *testing.T) {
		...
		_, accessibleFound := svc.modelsListCache.Get(modelsListCacheKey(&group9, accessibleModelsCacheNamespace+PlatformAnthropic))
		require.False(t, accessibleFound)
