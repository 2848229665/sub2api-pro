...
// legacy 引擎粘性写回（评审 M-Legacy 回归）：Fork 会在抢到槽位后原子
// 认领 canonical owner，但认领结果必须被标记为可回滚；终检否决时不能泄漏
// 新绑定。无门时保持 Fork 的 eager canonical-claim 与原 TTL 语义。
func TestProfitControl_LegacyEngineTracksStickyBindingUnderGate(t *testing.T) {
	now := time.Now()
	...
	t.Run("gated selection rolls back provisional binding on terminal rejection", func(t *testing.T) {
		...
		require.True(t, selection.stickyBindingClaimed, "Fork 的临时 canonical owner 认领必须可回滚")
		require.Equal(t, cheap.ID, cache.sessionBindings["openai:"+sessionHash])

		admissionCtx := ContextWithSelectionProfitGate(ctx, selection)
		require.NoError(t, svc.RejectAcquiredOpenAISelection(admissionCtx, &groupID, sessionHash, selection))
		require.Empty(t, cache.sessionBindings, "利润终检否决后不得泄漏临时粘性绑定")
	})

	t.Run("ungated selection keeps fork eager canonical claim", func(t *testing.T) {
		svc, cache := newSvc(map[string]int64{})
